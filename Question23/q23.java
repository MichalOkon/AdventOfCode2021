...
class Solution {
    ...
    public static void main(String[] args) {
        try {
            ...
            int foundPaths = checkPath("start", caveMap, new HashSet<>());
    ...
    public static int checkPath(String path, Map<String, List<String>> caveMap, Set<String> visited) {
        if(path.equals("end")) {
            return 1;
        }
        ...
        for(String cave : caveMap.get(path)) {
            if(!newVisited.contains(cave)) {
                sumOfPaths += checkPath(cave, caveMap, newVisited);
            }
        }
        return sumOfPaths;
    }
