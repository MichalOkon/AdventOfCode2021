...
class Solution {
    ...
    public static int findMinimum(ArrayList<Integer> positions, int from, int to) {
        ...
        while (max - min > 1) {
            ...
            int leftValue = calculateFuel(positions, middle - 1);
            int middleValue = calculateFuel(positions, middle);
            int rightValue = calculateFuel(positions, middle + 1);
        ...
        return Math.min(calculateFuel(positions, min), calculateFuel(positions, max));
    }

    public static int calculateFuel(ArrayList<Integer> positions, int point) {
        int fuel = 0;
        for (Integer position : positions) {
            int distance = Math.abs(position - point);
