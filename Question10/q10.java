...
class Solution {
    ...
    public static int fillGrid(ArrayList<Integer> inputArray, int[][] grid) {
        ...
        for (int i = 0; i < inputArray.size(); i += 4) {
            ...
            if (x1 == x2) {
                ...
                overlaps += insertDiagonalLine(grid, x1, x2, y1, y2);
            }
        }
        return overlaps;
    }
    ...
    public static int insertDiagonalLine(int[][] grid, int x1, int x2, int y1, int y2) {
        int newOverlaps = 0;

        Function<Integer, Integer> iterateX;
        Function<Integer, Integer> iterateY;
        if (x1 < x2) {
            iterateX = x -> x + 1;
        } else {
            iterateX = x -> x - 1;
